...
首次实施时，把 `~/.hermes/reminder-today.md` 里「待转录视频」一节的 3 条链接作为初始 todo，全部归入 `video` tab：

| 视频 | 初始 status | date |
|------|------------|------|
| 世界模型：在 AI 里抛硬币（B站） | `todo` | 实施日 |
| 中科院研究生用 AI 写论文（B站） | `todo` | 实施日 |
| GPT-5.6+image2 三步法输出学术 PPT（B站） | `todo` | 实施日 |

其他 tab（科研 / 编程 / 生活）首次提交时保持 `items: []`。`reminder-today.md` 的「待转录视频」一节在迁移后由用户自行删除，不再维护。